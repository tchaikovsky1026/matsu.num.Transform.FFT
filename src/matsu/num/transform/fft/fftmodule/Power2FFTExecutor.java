 ...
 * 
 * <p>
 * このクラスが提供する{@linkplain InnerDFTExecutor#compute(ComplexNumber[], FourierBasisComputer)}では, 
 * 例外スロー条件として次を追加する. <br>
 * サイズが2の塁乗でない場合に{@linkplain IllegalArgumentException}をスローする.
 * </p>
 * 
 * @author Matsuura Y.
 * @version 12.5
 */
public final class Power2FFTExecutor {

    private static final InnerDFTExecutor INSTANCE = new Power2FFTImpl();

    private Power2FFTExecutor() {
    ...
    private static final class Power2FFTImpl implements InnerDFTExecutor {
        ...
        private final class Power2FFTHelper {
             ...
             * @throws IllegalArgumentException サイズが2の累乗でない場合, 基底関数の生成器がdataのサイズに対応していない場合
            ...
            private ComplexNumber[] fftRecursion(ComplexNumber[] currentData) {
                ...
                if (N <= 4) {
                    return Power2FFTImpl.this.rawDFT.compute(currentData, this.fourierBasisComputer);
                }
                ...
                /* 横方向のDFT (サイズN1をN2個) */
                for (int j2 = 0; j2 < N2; j2++) {
                    b[j2] = Power2FFTImpl.this.rawDFT.compute(b[j2], this.fourierBasisComputer);
                }
