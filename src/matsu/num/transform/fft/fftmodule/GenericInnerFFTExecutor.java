 ...
 * 
 * <p>
 * このクラスが提供する{@linkplain InnerDFTExecutor#compute(ComplexNumber[], FourierBasisComputer)}の事前条件(引数の条件)は,
 * 標本サイズが大きすぎないことである. <br>
 * 標本サイズが{@linkplain #MAX_DATA_SIZE}を超過した場合は{@linkplain IllegalArgumentException}をスローする.
 * </p>
 * 
 * <p>
 * 標本サイズ(データサイズ)を<i>N</i>とし, <i>W</i> = exp(-i(2&pi;/<i>N</i>))とする. <br>
 * DFTでは, <i>k</i> = 0,...,<i>N</i> - 1として<br>
 * <i>A</i><sub><i>k</i></sub> = &Sigma;<sub><i>j</i>=0</sub><sup><i>N</i>-1</sup><i>a</i><sub><i>j</i></sub> 
 * <i>W</i><sup><i>jk</i></sup>, <br>
 * IDFTでは, <i>j</i> = 0,...,<i>N</i> - 1として<br>
 * <i>a</i><sub><i>j</i></sub> = &Sigma;<sub><i>k</i>=0</sub><sup><i>N</i>-1</sup><i>A</i><sub><i>k</i></sub> 
 * <i>W</i><sup>-<i>jk</i></sup> <br>
 * である.
 * </p>
 * 
 ...
 * @version 12.8
 */
public final class GenericInnerFFTExecutor {
    ...
    private static final InnerDFTExecutor INSTANCE = new GenericFFTImpl();

    static {
        MAX_DATA_SIZE = PrimeFFTExecutor.MAX_DATA_SIZE;
    }

    private GenericInnerFFTExecutor() {
    ...
    private static final class GenericFFTImpl implements InnerDFTExecutor {

        private final InnerDFTExecutor rawDFT = RawDFTExecutor.instance();
        private final InnerDFTExecutor primeFFT = PrimeFFTExecutor.instance();
        ...
        private final class FFTCalculation {
            ...
            private ComplexNumber[] fftLocal(ComplexNumber[] localData) {
                ...
                if (localData.length < PRIME_FFT_THRESHOLD) {
                    return GenericFFTImpl.this.rawDFT.compute(localData, this.basisComputer);
                }

                return GenericFFTImpl.this.primeFFT.compute(localData, this.basisComputer);
            }
        }
    }
}
