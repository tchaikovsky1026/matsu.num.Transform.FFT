...
public class GenericIDFTExecutorImplTest {

    public static final Class<?> TEST_CLASS = GenericIDFTExecutorImpl.class;
    public static final IDFTExecutor EXECUTOR_NEW = new GenericIDFTExecutorImpl(CommonLib.defaultImplemented());
    ...
    public static class IDFTの実行を検証_サイズ100 {
        @DataPoint
        public static final IDFTExecutor executorNew = EXECUTOR_NEW;

        @DataPoint
        public static final IDFTExecutor executorCall =
                FFTModuleExecutorProvider.byDefaultLib().get(ExecutorTypes.DFT.GENERIC_IDFT_EXECUTOR);
        ...
        @Theory
        public void test_DFTの実行(IDFTExecutor executor, ComplexNumberArrayDTO dataDTO) {

            ComplexNumber[] data = ComplexNumber.from(dataDTO.realPart, dataDTO.imaginaryPart);

            FourierBasisComputer basisComputer = COMPUTER_SUPPLIER.covering(data.length, FourierType.IDFT);
